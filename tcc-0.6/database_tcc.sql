...
-- Adicionar coluna GameCover na tabela fangames se não existir
ALTER TABLE `fangames` 
ADD COLUMN IF NOT EXISTS `GameCover` varchar(500) DEFAULT NULL AFTER `Genre`,
ADD COLUMN IF NOT EXISTS `SystemRequirements` text DEFAULT NULL AFTER `ReleaseDate`,
ADD COLUMN IF NOT EXISTS `UpdatedAt` datetime DEFAULT current_timestamp() ON UPDATE current_timestamp() AFTER `CreatedAt`,
ADD COLUMN IF NOT EXISTS `DownloadLink` text DEFAULT NULL AFTER `GameCover`,
ADD COLUMN IF NOT EXISTS `ReleaseDate` date DEFAULT NULL AFTER `SystemRequirements`;