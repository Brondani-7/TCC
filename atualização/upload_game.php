...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $developerId = $_SESSION['customer_id']; // Corrigido para customer_id

        // Processar tags
        $tags = [];
        if (!empty($_POST['gameTags'])) {
            $tagInput = trim($_POST['gameTags']);
        ...
        $stmt = $pdo->prepare("
            ...
            (GameTitle, GameDescription, DeveloperID, Genre, Franchise, Status, FileSize, GameFile, CreatedAt) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
        ");
        ...
        if (!empty($tags)) {
            foreach ($tags as $tagName) {
                ...
                $tagStmt = $pdo->prepare("SELECT TagID FROM game_tags WHERE TagName = ?");
                ...
                if ($existingTag) {
                    ...
                    // Criar nova tag
                    $tagStmt = $pdo->prepare("INSERT INTO game_tags (TagName) VALUES (?)");
                    $tagStmt->execute([$tagName]);
                    $tagId = $pdo->lastInsertId();
                }

                // Relacionar tag com o jogo
                $relationStmt = $pdo->prepare("INSERT INTO game_tag_relations (GameID, TagID) VALUES (?, ?)");
                $relationStmt->execute([$gameId, $tagId]);
            }
        }
